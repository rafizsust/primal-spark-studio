...
export async function getFromR2(
  key: string
): Promise<{ success: boolean; bytes?: Uint8Array; contentType?: string; error?: string; status?: number }> {
  const config = getR2Config();
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error("R2 get failed:", response.status, errorText);
      return { success: false, error: `R2 get failed: ${response.status}`, status: response.status };
    }
